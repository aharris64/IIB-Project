...
def multi_clahe(img, num, clip_limit=2.0, tile_grid_size=(8, 8)):
    img_clahe = img
    for _ in range(num):
        img_clahe = clahe(img_clahe, clip_limit, tile_grid_size)
    return img_clahe.astype(np.float32)
...
def percentage_based_enhancement(img, p_low=15, p_high=15):
    """
...
def percentile_controlled_gamma(img, pers=(2.5, 50), targets=(0.1, 1)):
    """
