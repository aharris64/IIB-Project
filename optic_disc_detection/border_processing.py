...
def create_mask(img, threshold_value=5/255, distance_to_edge=10, max_border_fraction=0.001):
    """
...
def border_fill_median(img, mask):
    """
    Fill pixels outside the mask with the median intensity of the valid region
    First idea - not used in main pipeline
    """
    mask_f = mask.astype(bool)
    fill_value = float(np.median(img[mask_f])) if np.any(mask_f) else 0.0
    filled = img.copy()
    filled[~mask_f] = fill_value
