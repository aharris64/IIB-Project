...
def find_DoG_candidates(pyramid, threshold=0.001): # Minima for Optic Disc
    """
    ...
    for o, octave in enumerate(pyramid):
        ...
        for i in range(1, len(DoG)-1):
            ...
            for y0, x0 in zip(ys, xs):
                ...
                # candidates.append(((x, y), o, i, sigma, float(D[y, x])))

            # h, w = D.shape
            # # TODO: Vectorize to make quicker
            ...
            #         # Etract 3x3 neighbourhood across space and scale
            #         patch = np.array([
            #             prevD[y-1:y+2, x-1:x+2],
            #             D[y-1:y+2, x-1:x+2],
            #             nextD[y-1:y+2, x-1:x+2]
            #         ])

            #         center = patch[1, 1, 1]

            #         # Only keep sufficiently negative responses (contrast threshold)
            #         if center > -threshold:
            #             continue

            #         # Remove center value so it does not compare with itself
            #         patch[1, 1, 1] = np.inf

            #         # Check if center is a local minimum in 3D (x, y, scale)
            #         if center <= np.min(patch):
            #             sigma = sigmas[i+1] 
            #             candidates.append(((x, y), o, i, sigma, float(center)))
