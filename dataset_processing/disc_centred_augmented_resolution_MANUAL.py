...
#
#   <stem><ext>              — original centre crop (degraded)
#   <stem>_hflip<ext>        — horizontal flip of centre crop (degraded)
#   <stem>_crop_tl<ext>      — offset crop: top-left (degraded)
#   <stem>_crop_tr<ext>      — offset crop: top-right (degraded)
#   <stem>_crop_bl<ext>      — offset crop: bottom-left (degraded)
#   <stem>_crop_br<ext>      — offset crop: bottom-right (degraded)
#
...
# ---- SET YOUR DESIRED LOW RESOLUTION HERE ----------------------------------
LOW_RES = 14  # e.g. 56, 28, 112 — image is downsampled to this then upsampled to OUT_SIZE
# ----------------------------------------------------------------------------

dataset_name = f"disc_centred_r{RADIUS_SCALE_FACTOR}_cl{''.join(str(c) for c in REJECT_CLASS)}_augmented_lowres{LOW_RES}"
...
def degrade_resolution(img: Image.Image) -> Image.Image:
    """Downsample to LOW_RES then upsample back to OUT_SIZE using LANCZOS."""
...
def make_centre_crop(img: Image.Image, cx: float, cy: float, side: float) -> Image.Image:
    """Centred square crop resized to OUT_SIZE then resolution degraded."""
    crop = crop_with_padding(img, centred_square_box(cx, cy, side)).resize(
        (OUT_SIZE, OUT_SIZE), Image.LANCZOS
    )
    return degrade_resolution(crop)
...
def make_offset_crops(
    ...
    """
    Four crops offset by ±OFFSET_FRAC * side in x and y, resolution degraded.
...
print(f"Generating low resolution dataset at {LOW_RES}x{LOW_RES} -> {OUT_SIZE}x{OUT_SIZE}")
